...
public class EdgeDetection {//sobel edge detection
	int[] input;
	int[] output;
	...
	public void edge_detection() {//constructor
		progress=0;
	}

	public void init(int[] original, int widthIn, int heightIn) {//initialize size
		width=widthIn;
		height=heightIn;
	...
	public int[] process() {
		...
		for(int x=(templateSize-1)/2; x<width-(templateSize+1)/2;x++) {//calculation for magnitude
			progress++;
		...
		for(int x=0; x<width;x++) {//calculation for direction
